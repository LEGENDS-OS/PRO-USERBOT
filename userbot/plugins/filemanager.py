...
async def _(event):
    ...
    OUTPUT = f"**Files in [KANNADIGABOT](t.me/NAAN_1_KANNADIGA) DOWNLOADS Folder:**\n"
...
async def _(event):
    ...
    if "|" in input_str:
        ...
        src = src.strip()
        dst = dst.strip()
    cmd = f"mv ./userbot/DOWNLOADS//{src} ./userbot/DOWNLOADS//{dst}"
    reply_to_id = event.message.id
    if event.reply_to_msg_id:
        reply_to_id = event.reply_to_msg_id
    time.time() + PROCESS_RUN_TIME
...
CmdHelp("filemanager").add_command(
    ...
    "ls_saved",
    None,
    "Gives the list of all files in Saved directory of your KANNADIGABOT server",
).add_command(
