...
if NORMAL_PIC:
    KANNADIGA = [x for x in NORMAL_PIC.split()]
    PIC = list(KANNADIGA)
...
async def promote(promt):
    ...
    KANNADIGAevent = await edit_or_reply(promt, "Promoting...")
    ...
    try:
        ...
        await KANNADIGAevent.edit(NO_PERM)
        return
...
async def demote(dmod):
    ...
    if not admin and not creator:
        ...
        return
    KANNADIGAevent = await edit_or_reply(dmod, "Demoting...")
    rank = "??????"
...
async def _ban(event):
    ...
    KANNADIGAevent = await edit_or_reply(event, "`Whacking the pest!`")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        return await KANNADIGAevent.edit(NO_PERM)
    try:
        reply = await event.get_reply_message()
        if reply:
            await reply.delete()
    except BadRequestError:
        return await KANNADIGAevent.edit(
...
async def nothanos(unbon):
    ...
    if not admin and not creator:
        ...
        return
    KANNADIGAevent = await edit_or_reply(unbon, "Unbanning...")
    ...
    try:
        ...
        await KANNADIGAevent.edit("Sorry I Can't Unban This Retard!")
...
async def kick(usr):
    ...
    if not user:
        ...
        return
    KANNADIGAevent = await edit_or_reply(usr, "Kicking...")
    try:
        await usr.client.kick_participant(usr.chat_id, user.id)
        await sleep(0.5)
    except Exception as e:
        await KANNADIGAevent.edit(NO_PERM + f"\n{str(e)}")
        return
    if reason:
        await bot.send_file(
            usr.chat_id,
