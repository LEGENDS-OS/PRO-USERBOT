...
async def get_users(event):
    ...
    KANNADIGA_chat = legen_.lower
    restricted = ["@NAAN_1_KANNADIGA", "@Kannadiga_bots"]
    KANNADIGA = await eor(event, f"**Inviting members from** {legen_}")
    if KANNADIGA_chat in restricted:
    ...
    if not sender.id == me.id:
        ...
        KANNADIGA = await edit_or_reply(event, "`processing...`")
    KANNADIGA = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await KANNADIGA.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await KANNADIGA.edit(
        "**⚜️[Ͳєямιиαℓ Տτατυѕ](https://t.me/NAAN_1_KANNADIGA)**\n\n`🔸Inviting Users.......`"
    )
    async for user in event.client.iter_participants(KANNADIGA.full_chat.id):
        try:
...
async def get_users(event):
    ...
    await KANNADIGA.edit("**TerminalStatus**\n\n`Collecting Users.......`")
