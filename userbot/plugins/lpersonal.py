...
OS = [
    ...
    "PLS DONT DISTURB HIM KANNADIGA IS BUSY NOW WHEN HE COME BACK HE REPLY U",
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        event = await edit_or_reply(event, "KANNADIGA")
        que[e] = []
        qeue = que.get(e)
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"『🔱 KANNADIGA ™🔱』IS STARTING GAME")
    else:
        ...
        appendable = [e]
        qeue.append(appendable)
        await event.edit(f"KANNADIGA {ALIVE_NAME}")
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        queue = que.get(e)
        queue.pop(0)
        await event.edit(f"KANNADIGA STOPED RAID {ALIVE_NAME}")
