...
class KANNADIGA_GlobalCollection_Json(BASE):
    __tablename__ = "KANNADIGA_globalcollectionjson"
...
KANNADIGA_GlobalCollection_Json.__table__.create(checkfirst=True)


def get_collection(keywoard):
    try:
        return SESSION.query(KANNADIGA_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
