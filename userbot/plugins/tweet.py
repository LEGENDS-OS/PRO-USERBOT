...
async def nope(KANNADIGA):
    ...
    if not KANNADIGA:
        if KANNADIGA.is_reply:
            ...
            await edit_or_reply(KANNADIGA, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(KANNADIGA))}")
